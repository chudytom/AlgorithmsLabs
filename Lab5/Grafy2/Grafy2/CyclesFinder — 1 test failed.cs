...
namespace ASD
{
    public class CyclesFinder : MarshalByRefObject
    {
        ...
        public bool IsTree(Graph g)
        {
            ...
            return false;
            //return !FindCycle(g, out Edge[] cycle);
        }
        ...
        public Edge[][] FindFundamentalCycles(Graph g, Graph t)
        {
            ...
            g.GeneralSearchAll<EdgesStack>(null, null, null, out int cc);
            if (!IsTree(t) || g.VerticesCount != t.VerticesCount || FindCycle(t, out Edge[] tempCycle))
            {
                throw new ArgumentException();
            }
            //bool isTree = false;
            //if (t.VerticesCount - cc == t.EdgesCount && cc == 1 && g.VerticesCount == t.VerticesCount)
            //{
            //    isTree = true;
            //}
            //if (isTree == false)
        ...
        public bool FindCycle(Graph g, out Edge[] cycle)
        {
            ...
            bool isCycle = false;
            bool[] visted = new bool[g.VerticesCount];
            ...
            Predicate<int> beforeV = delegate (int n)
            {
                ...
                {
                    if (visted[edge.To] && edge.To != fromVerts[n] && fromVerts[n] >= 0)
                    {
                        isCycle = true;
                        end = edge.To;
                    }
